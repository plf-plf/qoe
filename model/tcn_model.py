...
class TemporalConvNet(nn.Module):
    def __init__(self, num_inputs, output_size,features_size,num_channels=[128, 64, 32, 16], kernel_size=2, dropout=0.2):
        super(TemporalConvNet, self).__init__()
        layers = [] #初始化一个空列表，用于存储网络的各个层
        num_levels = len(num_channels) #计算网络深度，即有多少层

        # 假设输入序列长度为10，输出序列长度为2  output_size是你想要预测的未来时间点的数量
        # 我们需要定义一个合适的num_channels列表来满足这个要求
        # 例如，我们可以使用逐渐减少的通道数
        # in_channels=10 ; len(num_channels)是通道层数，一般是3层 这里是定义一个n层卷积
        for i in range(num_levels):
            dilation_size = 2 ** i #扩张卷积（Dilated Convolutions）
