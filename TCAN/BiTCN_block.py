...
# 这个BTCN没用到，从别的地方copy过来的，没改；bi-directional BTCN的实现在BiTCN_block.py中的TemporalConvNet类中
class BTCN(nn.Module):
    def __init__(self, nodes, groups, dropout):
        ...
        out_channels = nodes
        stride = 1
        # in_channels, out_channels, kernel_size, key_size, num_sub_blocks, \
...
class TemporalConvNet(nn.Module):
    def __init__(self, emb_size,output_size, num_channels, num_sub_blocks, temp_attn, nheads, en_res,
        ...
        for i in range(num_levels):
            ...
            layers += [TemporalBlock(in_channels, out_channels, kernel_size, key_size, num_sub_blocks, \
                temp_attn, nheads, en_res, conv, stride=1, dilation=dilation_size, padding=(kernel_size-1) * dilation_size, \
                vhdrop_layer=self.vhdrop_layer, visual=visual, dropout=dropout)]
        self.network_p = nn.Sequential(*layers)
        # 这是BiTCN模型
        self.network_b = nn.Sequential(*layers)
    ...
    def forward(self, x):
        ...
        if self.temp_attn:
            x_b,x_p = x,x
            for i in range(len(self.network_p)):
                x_p, attn_weight = self.network_p[i](x_p)
                # 翻转时间维度，训练，翻转回来
                x_re = torch.flip(x_b,dims=[1])
                x_b, attn_weight_b = self.network_b[i](x_re)
