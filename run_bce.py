...
def train(net, train_iter, valid_iter, epochs, learning_rate,device,save_path,data_type,model_type):
    ...
    focalloss_function = FocalLoss(alpha=0.85, gamma=2)
    ...
    for epoch in range(epochs):
        ...
        for x_train, decoder_input,y_train in train_bar:
            ...
            loss_bce = focalloss_function(y_train_pred_bce, y_train_bce).sum()
        ...
        with torch.no_grad():
            ...
            for x_valid, decoder_input, y_valid in valid_iter:
                ...
                loss_bce = focalloss_function(y_valid_pred_bce, y_valid_bce).sum()
...
if __name__ == '__main__':
    ...
    data_type='bj'
    model_type='TCAN'   #LSTM or TCN or TCAN or BiTCN
    ...
    features_size = 35  # 数据特征维度,删除的维度不包括进去；抖音：预处理：51、时序：143；微信：无时序：51；时序：165；
    ...
    # 加载日志
    log_file = make_print_to_file(save_path)
